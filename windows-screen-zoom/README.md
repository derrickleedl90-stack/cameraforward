...
2. Double-click **Run Screen Zoom.cmd**. It compiles `bin\ScreenZoom.exe` using the .NET Framework compiler on your PC and opens it. No administrator rights, Python, Node.js, or Visual Studio are needed.
3. Click **Start in 3 seconds**, then put your pointer over the detail you want to enlarge. Alternatively, press **Ctrl + Alt + Z** while using another app, release the keys, and wait one second.
4. The desktop freezes and starts at 125%. Use the wheel or the plus/minus keys to change zoom.
5. Press **Esc** to unlock. You can close the app from its main window.
...
Zoom ranges from 100% to 800% in 25 percentage-point increments. Returning to 100% keeps the view locked; use Esc to unlock. Moving the pointer does not move the magnified image. To choose a new focus point, unlock and start again. The app waits for all keys and mouse buttons to be released before locking.
...
1. Build and launch; start from the button and from Ctrl + Alt + Z over Notepad.
...
7. Close or end the process and confirm normal keyboard/mouse input is restored.
